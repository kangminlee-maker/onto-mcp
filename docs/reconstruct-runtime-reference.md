...
`reconstruct` has active contracts under `.onto/processes/reconstruct/`,
MCP/direct-call wiring, material-aware runtime helpers, and a bounded integral
runner. Code and document are the first partially wired runtime profiles.
Spreadsheet and database profiles are contract-active but runtime-planned until
their adapters are implemented; mixed targets are partial-composite only.

The current runner:

- classifies target material, writes and validates
  `reconstruct-run-control.yaml` before semantic artifacts are trusted, and
  admits the first run-control owner with atomic create semantics
- writes and validates `registry-verification-evidence.yaml` against the
  current reconstruct contract registry
- expands supported directory targets into per-member source observations
- writes and validates `source-safety-ledger.yaml` before LLM prompt
  materialization, and writes the initial source frontier
- runs reconstruct lens judgments and exploration synthesis through a
  configured LLM provider, validating available evidence refs
- authors and validates source-purpose authority, and confirms inferred purpose
  when required
- writes and validates `material-admission-ledger.yaml` so purpose-critical
  admission rows are consumed by candidate, seed, and maturation artifacts or
  closed as limitations
- writes and validates `seed-authoring-readiness.yaml` before ontology seed
  authoring so selected-purpose closure is deterministic while semantic
  adequacy remains owned by the seed author and downstream validators; ontology-
  domain category rows are diagnostic unless a selected-purpose closure row
  exists for that category
- sends seed authoring a compact selected-purpose projection instead of the
  full source-purpose candidate artifact
- preserves the first invalid `ontology-seed.yaml` and its validation as repair
  input when seed validation fails, runs one focused seed repair call over the
  failed validation sections, and revalidates the repaired seed
- passes exploration synthesis to downstream frontier, purpose, and candidate
  inventory authors as a compact observation-id projection instead of full
  evidence-ref objects
- computes deterministic metrics and writes `reconstruct-run-manifest.yaml`

After seed handoff validation it also projects the first-pass maturation
surface: `maturation-baseline.yaml`, `actionability-matrix.yaml`,
`maturation-question-frontier.yaml`, `maturation-closure-frontier.yaml`,
`maturation-authority-response.yaml`, `answer-support-ledger.yaml`,
`maturation-answer-claims.yaml`, `ontology-expansion.yaml`,
`maturation-source-delta.yaml`, `maturation-convergence-ledger.yaml`,
proof-boundary artifacts for query, visualization, and graph-exploration
surfaces, and `maturation-continuation-decision.yaml` with runtime validations.
Final re-question convergence is recorded inside the convergence ledger before
`actionable_ready` can be projected.

It writes pre-publication `claim-projection.yaml` and
...
values. The final `reconstruct-record.yaml` is assembled after
`claim-projection-validation.yaml` and final-output provenance validation pass;
otherwise terminal projection must be blocked or limitation-backed. The final
`reconstruct-run-manifest.post-publication-validation.yaml` is a
post-publication audit for the complete manifest after final output and record
refs exist.

The current public run path defaults to `direct_call` semantic authoring and
host-mediated confirmation. It fails loud when provider/model/credentials,
LLM-authored artifact shape, unsupported material, or runtime gates are
invalid. Optional reconstruct `domain` input admits that domain's
`competency_qs.md` into the run governing snapshot for domain competency trace
validation; there is no separate active domain competency selection artifact.
...
both full and minimal seed authoring time out the run fails closed; runtime
does not author deterministic ontology seed content. Competency-question
timeout recovery may still project coverage questions from allowed claims and
domain competency rows so validation can prove coverage or preserve explicit
limitations.

## Artifact and gate catalog
...
## Authority and projection notes
...
receive compact prompt projections, such as selected observations with
shortened text excerpts, `source_purpose_projection` for the selected purpose
candidate, `ontology_seed_summary` for claim and competency-question authoring,
and `allowed_claims` for claim realization, while validation still checks all
...
mutable alias or another session's same-basename snapshot. The pre-handoff
terminal projection treats the post-maturation gate as not applicable until the
post-maturation snapshot exists; terminal validation also projects
`source_scout_pack_post_maturation_gate` when a manifest contains the
...
The scout projection prioritizes actor/action/state inspection without
replacing evidence refs or selected-purpose authority. For mixed targets, the
current scout scope may record `member_scoped_composite`, but it emits no scout
signal rows and carries an explicit limitation until a member-scoped scout
...
Terminal projection uses
`handoff-decision-validation.yaml.readiness_projection` as the seed iteration
readiness authority and requires atomically admitted valid run-control, valid
registry verification, validated handoff, and a validated pre-handoff
run-manifest snapshot. Run-control write transactions currently record
post-write `observed_file_hash` evidence; they do not claim every writer used
atomic rename. Public claim surfaces use `claim-projection-validation.yaml`, so
seed handoff readiness and actionable ontology readiness remain separate
claims. The final `reconstruct-run-manifest.post-publication-validation.yaml`
is the post-publication audit for the complete manifest after
`final-output.md` and `reconstruct-record.yaml` refs are known; it is not a
prerequisite for the pre-handoff seed iteration readiness projection, but it is
the required terminal authority before run-control can record durable
completion or promote `resume_allowed`. The same artifact records
`gate_projection[]`, where each active gate is evaluated through the registry
`required_when_predicate_catalog` before validation status is required.
...
- Full seeding and maturation plan:
  `.onto/processes/reconstruct/ontology-seeding-and-maturation-design.md`
- Active contract, source profile, lens judgment, artifact, gate, seed
  iteration readiness, and projection authority registry:
