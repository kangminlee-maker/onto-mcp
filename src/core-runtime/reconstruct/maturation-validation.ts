...
export function validateAnswerSupportLedger(args: {
  ...
  const violations: ReconstructMaturationValidationViolation[] = [];
  // G(a) slice 21 — record the obligations this validator genuinely enforces (RECORD 5/9). The
  // other 4 stay parked (see obligation-coverage-ledger.yaml notes): contradictions-bounded is
  // enforced by validateMaturationAnswerClaims (DELEGATED), the frontier→valid-lineage-index-
  // validation check is an unscoped global precondition (PARTIAL), and the external/runtime +
  // generic support-mode-required-refs obligations have no distinct, non-overlapping enforcement
  // here. Stamped before any per-cluster guard so the recorder fires on zero-row input.
  const assertedObligationIds: string[] = [];
  assertObligation(assertedObligationIds, "validate_evidence_cluster_question_refs");
  assertObligation(assertedObligationIds, "validate_user_confirmation_support_mode");
  assertObligation(
    assertedObligationIds,
