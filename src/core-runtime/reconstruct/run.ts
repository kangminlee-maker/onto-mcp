...
function calculateMetrics(args: {
  ...
  const gateFailureCount =
    validationStatus.source_observation_directive === "valid" &&
...
function createRunManifest(args: {
  ...
  return {
    ...
    happy_path_scope: {
      ...
      deferred_reason:
        "The current runner is a bounded happy path; domain context selection, failure classification, and revision loops require additional host/user semantics.",
    },
    steps: [
  ...
  return {
    ...
    async writeFinalOutput(input) {
      const confirmedClaims = allClaims(input.seedCandidate).filter((claim) =>
        input.seedConfirmation.confirmed_claim_ids.includes(claim.claim_id)
      );
