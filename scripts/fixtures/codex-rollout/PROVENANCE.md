...
Note that `sent` and `recv` counts do **not** match in any of the three: an exec that calls no tool
still produces a received record. §11-L1's bidirectional check cannot be a naive count comparison.

## Phases NOT covered

`session_meta.cli_version` is `0.145.0` in all three, and the MCP server in all of them is a synthetic
`probe` server — **not** the real `observation_read_facade`. Still missing, and therefore counted as
unverified for any parser that assumes them: **concurrent calls** (`Promise.all`), **multiple `text()`
calls**, **overlapping outer execs**, and **anything from the real façade**.
