...
- **규칙**: settings.json에서 선택 가능한 LLM 모델은 권위 레지스트리 [.onto/authority/supported-models.yaml](.onto/authority/supported-models.yaml)에 등록된 `(provider, model)`로 한정한다. 레지스트리에는 벤치마크 기록이 파이프라인 완주를 입증한 모델만(사람 큐레이션, 벤치마크 record 인용) 올린다. role 제한 엔트리는 `roles`에 명시된 dispatch role에서만 선택 가능하고, `roles`가 없는 기존 엔트리는 grandfathered full-route allowance로만 해석한다. 미등록 모델 또는 role 불일치 선택은 supported-model 게이트가 reconstruct live 실행 경계(실제 provider 호출)와 G7 가드(커밋된 모든 seat)에서 fail-loud로 거부한다. 단, B7 벤치 후보 예외는 settings/API/MCP 표면이 아니라 벤치 하니스가 runtime gate에 명시적으로 전달하는 option이며, **미등록 pair**와 **등록 pair의 role-확장 후보**(entry가 요구 role을 cover하지 않는 seat에서 그 role의 evidence를 수집하는 run 전용; 2026-07-17 owner 지시로 확장)를 exact allowlist route에서만 통과시킨다. unresolved provider/model과 제품 요청 표면은 이 예외가 구제하지 않는다. review 런타임 강제는 후속 과제이므로 런타임 게이트는 현재 reconstruct live 경로와 B4 벤치 후보 preflight에 배선돼 있고, review의 커밋 seat은 G7가 커버한다. settings 해석(`resolveSettingsChain`)은 순수 projection이며 이 게이트를 적용하지 않는다(게이트≠projection — mock/test 해석이 임의 fixture 모델로 통과할 수 있게).
- **근거**: 검증되지 않은 모델로 운영하면 파이프라인 안정성·품질 기준선이 무근거로 흔들린다. "지원함"은 벤치마크 증거로만 확립한다.
- **현재 source**: 권위 = `.onto/authority/supported-models.yaml`. runtime 게이트 = `assertSettingsModelsSupported`([settings-chain.ts](src/core-runtime/discovery/settings-chain.ts)) — 멤버십과 role 적합성, B7 벤치 후보 exact-route allowance는 [supported-models.ts](src/core-runtime/discovery/supported-models.ts)의 `assertSupportedModelRoutes`가 소유한다. listable role 집합은 같은 모듈의 `CONTRACTED_ROLES`가 소유하며, G7이 registry entry의 `roles` 선언·grandfathered 예외·role별 evidence record binding을 검증한다. 호출 지점: reconstruct live 실행 경계(`!mockRealizationEnabled`, [reconstruct-api.ts](src/core-api/reconstruct-api.ts)), B4 벤치 후보 preflight(`assertB4BenchCandidateDispatchAllowed`), G7 가드. `resolveSettingsChain`은 게이트를 적용하지 않는다.
- **강제**: 역량 경계(reconstruct live 실행 경계에서 fail-loud 거부) + G7 가드. 레지스트리의 `roles`는 G7 + `CONTRACTED_ROLES`가 소유하는 role-certification 표면이며 G4 보호 키가 아니다. 레지스트리의 `context_window_tokens`(reconstruct projection 예산 SSOT)와 `max_output_tokens`(direct-API output headroom 상한 SSOT)는 별도 model-spec 값이므로 추가로 **G4 보호** — 변경 시 `INVARIANT-CHANGE: INV-MODEL-1` 마커 필요. `↔ AGENTS §0-2`(.onto/settings.json 스키마/계약 변경은 사람 승인).
- **검증**: `npm run check:supported-models` — 커밋된 .onto/settings.json의 모든 모델 route가 등록된 `(provider, model)`과 필요한 role에 맞는지 검사하고, role-restricted 엔트리의 evidence binding을 재계산해 위반 시 비-0. 또한 `benchCandidate` 류 토큰이 제품/API/MCP/settings 표면으로 새지 않는지와 B4 live harness가 candidate config 구성 전에 runtime helper를 호출하는지 검사한다. model-spec token-limit 필드는 provenance를 요구하며 G4(`check:invariant-change`)가 마커 동반을 강제한다.
...
| G7 지원 모델 가드 | INV-MODEL-1 | `npm run check:supported-models` (커밋된 settings.json ⊆ supported-models.yaml; runtime도 reconstruct live 실행 경계에서 동일 게이트 `assertSettingsModelsSupported` 호출; B7 벤치 후보 token/binding guard 포함) |
...
| G11 reconstruct terminal-signal rethrow | INV-SCHEMA-1 | `npm run check:graceful-signal-rethrow` + G6 + CI (run/leaf-reader의 typed terminal catch가 비공집합이며 구조적으로 rethrow됨을 강제) |
