...
- **강제**: 역량 경계(reconstruct live 실행 경계에서 fail-loud 거부) + G7 가드. 레지스트리의 `roles`는 G7 + `CONTRACTED_ROLES`가 소유하는 role-certification 표면이며 G4 보호 키가 아니다. 레지스트리의 `context_window_tokens`(reconstruct projection 예산 SSOT)는 별도 모델-spec 값이므로 추가로 **G4 보호** — 변경 시 `INVARIANT-CHANGE: INV-MODEL-1` 마커 필요. `↔ AGENTS §0-2`(.onto/settings.json 스키마/계약 변경은 사람 승인).
- **검증**: `npm run check:supported-models` — 커밋된 .onto/settings.json의 모든 모델 route가 등록된 `(provider, model)`과 필요한 role에 맞는지 검사하고, role-restricted 엔트리의 evidence binding을 재계산해 위반 시 비-0. 또한 `benchCandidate` 류 토큰이 제품/API/MCP/settings 표면으로 새지 않는지와 B4 live harness가 candidate config 구성 전에 runtime helper를 호출하는지 검사한다. window 필드는 G4(`check:invariant-change`)가 마커 동반을 강제한다.
