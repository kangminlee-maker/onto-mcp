...
- `.onto/processes/reconstruct.md` + `process.md` 기존 위반 전수 수정 완료: Explorer / Adjudicator / Synthesize + review lens / synthesize 프롬프트에서 `Respond in {output_language}` 제거 → `Respond in English` + render seat 참조
...
- **Standard `[Language Policy]` prompt boilerplate 전수 주입 완료** — `process.md` 4 템플릿 + `.onto/processes/reconstruct.md` 3 agent prompt + `.onto/processes/review/lens-prompt-contract.md` + `.onto/processes/review/synthesize-prompt-contract.md` example skeleton 에 §7 canonical 블록 주입. 기존 inline "Respond in English" 지시와 병행해 LLM attention redundancy 확보
