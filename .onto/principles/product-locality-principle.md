...
| 설치 형태 | 경로 | 용도 |
|---|---|---|
| **product-local** | `{product}/node_modules/onto-mcp/` | product 에 버전 고정된 설치 |
| **global** | 글로벌 `node_modules/onto-mcp/` (`npm install -g`) | 주체자 전역 설치 |
...
onto CLI 진입 스크립트(`bin/onto`)가 실행될 때, 현재 product 에 로컬 설치(`node_modules/onto-mcp`)가 존재하면 **로컬 바이너리에 실행을 위임**한다. 글로벌 설치는 로컬 설치가 없을 때만 사용한다.

이 우선순위는 product 가 고정한 버전의 roles, authority, contract가 항상 사용되도록 보장하기 위한 것이다 (OaC authority chain 무결성).

해석 순서:

1. product 의 `node_modules/onto-mcp/bin/onto` 존재 → 로컬에 위임
2. 존재하지 않음 → 현재(global/development) 바이너리가 직접 실행

위임 시 `ONTO_HOME` 환경변수는 로컬 설치의 심링크 해석 전 경로(`node_modules/onto-mcp/`)로 강제 설정된다. realpath로 해석하지 않는다. 이는 위임된 프로세스가 global 설치의 리소스를 참조하는 것을 구조적으로 방지한다.
...
공유 자산의 읽기 참조와 `ONTO_HOME` 격리는 별개다. §3 #4의 격리 대상은 설치에 포함된 리소스(.onto/roles/, .onto/authority/)이며, 주체자 수준 공유 데이터(`~/.onto/domains/`)는 product-override 규칙으로 관리된다.
...
**쓰기 흐름**: 운영/긴급 데이터만 글로벌에 직접 기록된다. 그 외 기능은 product 에 먼저 기록한다.
...
§2.2에 따라 product `.onto/` 아래에 저장되어야 한다. §2.3 운영/긴급 데이터 예외에 해당하는지 확인하고, 글로벌에 직접 쓰는 기능은 §2.3 테이블에 예외로 등록해야 한다.
