...
4. **Completeness / anti-fooling — ENFORCEABLE static sweep** (xval #0 + onto issue-001/002/004/007/008 — the decisive cluster; Q-C CLOSED — implemented as the guard's clause 6, empirically verified to fail on an injected 9th heading):
   - **(static, the enforceable 9th-section catch)** the guard asserts (a) EVERY markdown `## ` heading literal in run.ts is the exact `` `## ${FINAL_OUTPUT_SECTION_HEADINGS.<key>}` `` template (so an inline, re-inlined, or NEW non-module heading fails), and (b) the count of `appendFinalOutput*` emitter definitions equals the module section count (so a new emitter fn fails). run.ts has no prose `## `, so the sweep is exact today; a future legitimate `## ` prose line surfaces here and must be reconciled (the same source-text boundary G8 R5 documented). A module-internal check also asserts every `FINAL_OUTPUT_SECTION_HEADINGS` constant is used by a descriptor (no orphan heading).
   - **(behavioral, per-path confirmation)** a test activates all 3 conditional append paths (`documentProjectionTruncations` + `workbookInventoryProjectionTruncations` + disclosed revisions — xval #6, the 2 truncation emitters now exported) and asserts the emitted output's `## ` heading SET equals the module's 3 conditional headings; the 5 bound headings are confirmed by the existing run.test.ts pipeline assertions.
   - §5's "a 9th section without a registry row fails CI" is enforced by the static sweep (the behavioral test confirms the conditional emit path).
...
- Adding/renaming/dropping a section (or changing a heading at the emitter OR binding, or adding a 9th append section) without updating the registry fails CI (G9): an emitter-heading rename is caught (no inline literal allowed → it must change the module constant → parity vs registry fails); a section_id rename is caught (parity + the derived required_fragments); a new section is caught (static sweep: heading-form + emitter-count); a bindings-array reorder is caught (clause 7).
...
- **Implementation review** `wf_d2e86462-d2d` (6 lenses → adversarial verify, post-build, on commit 16c0485): 7 confirmed mediums — all one class: the §3.1.4 completeness mechanism was *documented as built but unbuilt* (the guard only checked the 8 known headings for re-inlining; a NEW 9th heading passed, empirically proven; the "behavioral matrix" was 3 isolated `toContain` checks; no bindings-order assertion; clause-4 untested). Fixed in the follow-up commit: the static sweep (heading-form + emitter-count) is now the real enforceable 9th-section catch (negative-tested incl. the injected case), the bindings-array order is a guard clause, clause-4 has a negative test, the behavioral test is a real conditional heading-SET equality, and the guard docstring + §3.1.4/§5 now describe what is actually enforced. Lesson: the declared-not-enforced class this track closes recurs in the track's own guard implementation — the post-build adversarial review caught it before PR.
