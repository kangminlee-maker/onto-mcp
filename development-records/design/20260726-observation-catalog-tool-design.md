...
`--ignore-user-config`와 전용 `CODEX_HOME` **양쪽에서 살아남는다** — 사용자 config가 아니라
...
- **목표인 것** —
  1. **설정이 통제하는 것은 실제로 제거한다** (`day1-mcp` Workspace 쓰기, `onto_reconstruct` 재귀)
  2. **남는 것을 기동 시 열거해 기록한다** — 지울 수 없다면 최소한 **모른 채로 돌지는 않는다**
...
| **0b** | **전용 `CODEX_HOME`으로 워커 기동 + 기동 시 표면 열거·기록 + 금지 능력 검사** | 적대 프롬프트로 도구 열거 → 집합이 비어있지 않음을 단언(공허 통과 방지). `day1-mcp`·`onto_reconstruct` **부재를 개별 확인**(설정이 통제하는 것은 실제로 제거됐는가). 남은 provider-side 능력은 **기동 시 기록**한다 — 제거는 불가하되 무지는 허용하지 않는다(§5.4). **설계 채택과 무관하게 값 있음** |
