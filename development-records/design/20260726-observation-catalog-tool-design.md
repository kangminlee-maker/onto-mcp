...
codex의 상속 MCP 설정을 **병합이 아니라 대체**하고(claude 경로의 `--strict-mcp-config` 전례),
기동 시 **실제로 받은 도구 목록을 화이트리스트와 대조**한 뒤에 프롬프트를 보낸다. 불일치면 중단.

> 이 단계는 **관측 도구 설계의 채택 여부와 무관하게** 값이 있다. 두 초안이 독립적으로
> 같은 결론에 도달했다.
>
> 설치된 codex가 상속 MCP 설정을 신뢰성 있게 대체하지 못하면 **이 설계는 출하하지 않는다.**
> "위험한 도구를 부르지 말라"는 프롬프트 지시는 대체재가 아니다.
...
| 단계 | 내용 | 검증 |
|---|---|---|
| **0** | **워커 MCP 표면 대체 + 기동 시 화이트리스트 대조** (§5) | 적대 프롬프트로 도구 열거 → 집합이 비어있지 않고 정확히 허용 목록과 같음을 단언. **설계 채택과 무관하게 값 있음** |
...
- **큰 페이로드와 함께일 때도 워커 도구가 살아 있는가** — 미측정. 프로브는 작은 프롬프트였다.
  단계 0에서 실측한다.
- codex가 상속 MCP 설정을 **완전히** 대체할 수 있는가 — 미확인. **출하 차단 조건**(§5.2)
