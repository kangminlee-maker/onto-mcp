...
#### 6차 교차검증 — 렌즈 2벌 (2026-07-27)

렌즈 I(전체 diff)는 **네트워크 오류로 판정 없이 죽었다**(583,648 토큰 소진 후 stream disconnect) —
재발행 필요. 렌즈 J(**5차 수정 자체**, `xhigh`)는 material 1건 + low 1건.

| 발견 | 재현 | 처분 |
|---|---|---|
| **J1 medium — 5차 수정이 4차의 성질을 되돌렸다.** 혼합 코퍼스를 통과시키려고 오라클을 "미지의 키 없음"으로 완화하면서 **전 행 정확 키 집합**을 잃었다. 리뷰어 구성: `location`을 index 0 이후 전 행에서 지워도 통과한다(정책 문구는 `location on some rows only`로 조용히 바뀔 뿐) | 확인 | **수정**: 완화 대신 **행별 기대치를 실제 tail 투영기로 파생**(`expectedNavigationKeys`) — 규칙을 재서술하지 않고 `projectBreadthFoldTailRung`을 그 관측의 one_line 행에 돌린다. 호출자는 `exactKeys`(균질) 또는 `rung`(혼합) 중 하나를 **반드시** 준다(둘 다 없으면 throw). 변이 M22로 확증: 리뷰어가 지목한 그 결함이 이제 7개 테스트를 실패시킨다 |
| **J2 low — 공통 키가 없는 코퍼스에서 문구가 `; …`로 시작** | 확인 | 리뷰 도착 전에 주 세션이 같은 것을 찾아 수정(제품 도달 불가 — 투영기가 항상 `observation_id`를 낸다 — 이지만 문장이 아니었다) |

**리뷰어가 공격해서 결함 없음으로 확인한 것**: 혼합 문구가 예산 경계에서 등급 선택을 바꿀 수 있으나
**측정=디스패치이므로 옳다** · 유효 production 사다리는 여전히 비증가(중복 `location` 삭제가 pretty JSON에서
최소 23바이트를 줄이고 문구는 18바이트를 더한다) · 혼합 테스트는 한 모양으로는 통과 불가 ·
`offExposureFor`는 production의 순서·개수와 일치하고 prioritized 접두사 성질은 probe 한정이 아니라 보편 ·
1~4차 replay 성질 중 깨진 것 없음.

**교훈**: **완화는 수정이 아니다.** 5차가 혼합 코퍼스를 통과시키려고 검사를 느슨하게 했고, 그것이
4차가 세운 성질을 되돌렸다. 옳은 처방은 검사를 **약하게** 만드는 것이 아니라 **기대치를 피검 코드의
정본 함수로 파생**하는 것이었다 — 이 트랙에서 네 번째로 같은 형태의 처방이다.

#### 7차 교차검증 — **material 0** (2026-07-27)

렌즈 K = 전체 diff 재발행(6차에서 네트워크로 죽은 것) · 렌즈 L = **6차 수정 자체**. **둘 다 material 0 ·
반박된 주장 0.**

렌즈 L이 6차 수정을 공격해 확인한 것: ①one_line 재구성이 production 투영기와 **일치**한다(투영기가 다섯 키를
무조건 내고 아티팩트 계약이 전부 string이므로, `key in observation` 재구성이 JSON 파싱 결과와 같다)
②기대치를 같은 투영기로 파생하는 **순환은 경계가 있다** — 투영기 자체의 규칙은
`source-breadth-fold.test.ts`가 독립으로 고정하고, 이 오라클은 "작성자가 그것을 옳게 썼는가"를 본다
③등급을 거짓으로 넘기는 것은 가능하나 커밋된 호출자는 전부 안전(고정 리터럴 또는 폴드가 만든 공시)
④정확 키 검사가 **정당한 production 모양을 거부하지 않는다**(세 모양 전수 확인) ⑤공통 키 없는 분기는
옳고 production 도달 불가 ⑥1~5차 성질 중 깨진 것 없음.

**정직한 한정**: 두 렌즈는 같은 모델(`gpt-5.6-sol`)의 서로 다른 렌즈·effort다 — 동종 수렴이라
공유 맹점이 남을 수 있다. 그래서 이 clean은 **변이 22종 전탐지 · 두 트리 parity 3코퍼스 · replay 6팔+
자기점검 · 주 세션 독립 재도출**과 함께 읽어야 하며, 그것 하나로 검증을 대체하지 않는다.
렌즈 L은 read-only 샌드박스 때문에 vitest를 못 돌렸고(직접 런타임 프로브로 대체) 스위트 실행은 주 세션이 했다.
