...
> 상태: **구현 완료 (Steps 1–9, 2026-06-09)** — branch `feat/host-orchestration-stage1`, lens 슬라이스 범위(사용자 승인). 런타임(durable step engine 4a–4e + A/B fail-closed 5 + API/MCP 6 + reference host 7) + 권위 문서(rank-1 개념 8 + rank-5 계약 8) 정합. 후속: deliberation/synthesize/runtime-owned 단계 host 구동 → `completed` 전체 파이프라인. 짝 문서: [Phase 2 로드맵](./phase2-host-orchestration-roadmap.md).
...
4. **(strangler) durable step engine 구축 후 B 조립** — 각 추출은 **기존 단위테스트 + E2E로 동작 보존 검증**(A 무회귀 게이트). 신규 모듈 `src/core-runtime/review/review-execution-steps.ts`.
   ...
   - **4b `validateUnitSeatToResult` + `mergeUnitResultIntoExecutionResult`** 추출. `executeReviewPromptExecution`의 결과 경로를 이 함수 호출로 교체. 게이트: 기존 review-api/issue-artifact 테스트 + 신규(host seat→`completed` result, 잘못된 seat→`failed`). **[landed 2026-06-09 — additive]** 두 함수를 `review-execution-steps.ts`에 추가(seat→`ReviewUnitExecutionResult` 투영 + execution-result.yaml에 unit_kind별 bucket upsert). 아키텍처 제약상 `review/`는 `cli/`를 import하지 않으므로 seat 검증은 review-level primitive(비어있음 검사 + sidecar 구조 검증)만 사용한다 — `validateUnitOutputFile`의 cli-only 헬퍼(markdown contract/size)는 차용하지 않음(B Stage 1 seat-gate로 충분; 심층 stage validator는 기존처럼 downstream 소비 지점이 적용). A의 batch-write→incremental-merge 전환은 **4f rebase(Stage 1 범위 밖)**이라 `executeReviewPromptExecution`은 4a처럼 무접촉 → A 무회귀는 자명. timing은 host 소유라 result에 `timestamp_provenance: "batch_window"`(비교 불가) 각인.
   - **4c `ensureUnitPacket(sessionRoot, unit)`** 추출(lens=noop; issue-artifact=`writeIssueArtifactPromptPacket`; delib/synth=worklist→packet). A의 인라인 packet 생성을 호출로 교체. 게이트: 기존 테스트 + E2E(A 불변). **[landed 2026-06-09 — additive]** `ensureUnitPacket`: lens→noop(prepare가 만든 packet 존재 검증·반환); issue_artifact→`writeIssueArtifactPromptPacket`에 durable 입력 위임. 입력 재구성(`reconstructIssueArtifactPacketInputs`): `project_root`는 session-metadata(plan의 `session_metadata_path` 경유)에서, `lensOutputPaths`는 ledger의 **trusted lens unit** outputRefs에서, problem-framing의 deliberation refs는 trusted deliberation unit + plan의 teamlead `deliberation_output_path` + `resolveProblemFramingProfileRef`에서. 공유 `buildSessionLedger` 추출(computeReviewFrontier와 공용). **delib/synth packet은 fail-closed**(생성기가 deliberation/synthesis 실행 orchestration과 인라인 entangled — finding-ledger는 sidecar 모드서 runtime 생성, issue-ledger runtime completion, stance-matrix collection 등 packet≠host-exec 분기 존재 → divergent packet 방지 위해 4e 라운드 조립에서 처리). A의 인라인 생성은 4f rebase 영역이라 무접촉 → A 무회귀 자명. 테스트 6개(lens noop present/throws; delib fail-closed; reconstruct lens paths+project_root, untrusted 제외; issue_artifact 실제 packet 생성).
   - **4d `finalizeStageGate(sessionRoot)`** 추출(`writeLensCompletionBarrier` 래핑, A·B 공용). 게이트: barrier 테스트. **[landed 2026-06-09 — 첫 A·B 공유 추출]** barrier 의미론(planned/completed/failed→missing/degraded/status/downstream_allowed)을 **순수 함수 `computeLensCompletionBarrier`**로 `review/lens-completion-policy.ts`에 추출. **A의 `writeLensCompletionBarrier`를 이 순수 함수 호출로 교체**(동작 보존 strangler swap — id 파생/파일쓰기/로그는 그대로, 게이트 계산만 공유) → 개념 경제상 barrier 의미론 중복 제거. `finalizeStageGate(sessionRoot)`(review-execution-steps.ts): ledger에서 completed=trusted lens unit, failed=status failed lens unit, planned=plan lens seat 재구성; min=`resolveRequiredParticipatingLensCount`; observedDispatchWidth=`plan.max_concurrent_lenses ?? planned count`; `computeLensCompletionBarrier` 호출→barrier 파일 기록·반환. halt/proceed는 호출자(4e)가 `downstream_allowed`로 판단. 테스트 2개(전 lens trusted→passed/allowed; lens 누락→failed/blocked, missing 포함). full vitest 74files/878 pass(A 무회귀 — 순수 추출).
   - **4e `reviewRound`/`reviewAdvance`(B)** 를 4a–4d + `completeReviewSession` 위에 조립(+ orchestration-owner 검사 = Step 5 연계). 게이트: B 단위테스트(mock seat → 단계별 frontier 전진 → `completed`) = 결정론 reference-host 슬라이스(Step 7 선행). **[landed 2026-06-09 — 라운드 엔진 + lens 슬라이스 (사용자 승인 범위)]** `reviewRound(sessionRoot)`=`computeReviewFrontier`+frontier 각 unit `ensureUnitPacket`→ready units 반환(실행 X). `reviewAdvance(sessionRoot, executed, {base?})`=각 `validateUnitSeatToResult`+`merge`→`finalizeStageGate`→다음 round 결과. 반환 `ReviewRoundResult`={in_progress, ready_units}|{ready_to_assemble}|{halted}. **orchestration-owner fail-closed**: round/advance 진입에서 session-metadata `orchestration!=="host"`면 거부(capability surface, Step 5의 advance측). `assembleIfComplete`은 `completeReviewSession`(cli)이라 review/서 직접 호출 불가 → reviewAdvance는 terminal 시 `ready_to_assemble` 신호만, 실제 assemble은 core-api wrapper(Step 6). 첫 advance의 execution-result scaffold(base)는 execution-level metadata derive가 cli라 core-api/테스트가 주입. **lens 슬라이스 결정론 E2E**: fresh host 세션→reviewRound=lens ready units; mock seats→reviewAdvance([lens ids])→validate+merge+gate→다음 frontier=finding-ledger(packet까지 ensure). 테스트 3개(round=lens; advance→issue-artifact 전진; runtime 세션 거부). **deliberation/synthesize/runtime-owned(stance-matrix·sidecar finding-ledger·issue-ledger completion) 단계의 완전한 B 구동은 후속**(4c entanglement: 생성기가 A cli 실행 orchestration에 인라인 → 추출 필요). additive(A 무접촉). full vitest 74files/881 pass.
...
5. **A/B fail-closed 경계**: `executeReviewPromptExecution` 진입 거부(host), `reviewAdvance` 진입 거부(runtime). 게이트: 누수 거부 테스트(완료기준 4). **[landed 2026-06-09]** capability-surface 강제를 **순수 guard 단일화**로 구현: 신규 `review/orchestration-owner.ts` — `assertHostOrchestratedSession`/`assertRuntimeOrchestratedSession`/`resolveOrchestrationOwner`(미stamp→runtime 기본). **B측**(이미 4e): round/advance 진입의 `assertHostOrchestration`이 session-metadata stamp 읽어 위임. **A측**(이번): `executeReviewPromptExecution` 진입에서 boundary assertion 직후 `assertRuntimeOrchestratedSession(executionPlan.orchestration)` — spawn 전 host 세션 거부. stamp는 execution-plan/session-metadata(materializers.ts)에 각인됨. 양방향 누수 거부 = orchestration-owner.test.ts 순수 테스트(host→A guard throws; runtime/undefined→B guard throws); A측 runtime 통과(false-reject 없음)는 full vitest+invocation-runner conformance가 증명. 75files/886 pass.
6. **MCP 도구**: `onto_review_round`/`onto_review_advance` 등록(+ 이름 charter). 게이트: typecheck + 도구 스키마. **[landed 2026-06-09]** **6a** core-api: `OntoReviewCoreApi.reviewRound`/`reviewAdvance`가 review-execution-steps 코어에 위임. core가 첫 advance에 execution-result scaffold 자체 생성(`buildInitialExecutionResultScaffold` — plan에서 execution-level metadata derive, status `halted_partial` 진행중 proxy; 헬퍼가 review/discovery라 cli 의존 없음 → wrapper의 base 주입 불필요). reviewAdvance는 `ready_to_assemble`시 `completeReviewSession`(cli assemble) 자동 실행→`assembled` 반환. 신규 타입 ReviewRoundRequest/ReviewAdvanceRequest/ReviewRoundResult. 테스트: review-api.test.ts에서 host 세션(orchestration=host, markdown lens) prepare→round(lens units)→mock seats→advance→finding-ledger frontier(packet ensure). **6b** MCP: `onto_review_round`/`onto_review_advance` 등록(server.ts TOOL_DEFINITIONS+handler case+raw JSON schema; tool-schemas.ts zod schema+OntoToolNames; usage resource에 host-orchestration 안내). assemble 분기는 wired but full pipeline(synthesize)에서만 도달 — lens 슬라이스 밖. typecheck+full vitest 75files/888+mcp:review conformance green.
7. **reference host driver**(live+mock). 게이트: mock 모드 결정론 E2E → `completed` 고정 record. **[landed 2026-06-09 — lens 슬라이스 mock]** 신규 `cli/host-orchestration-reference-driver.ts`: `driveHostOrchestration({sessionRoot, executeUnit, maxRounds})` — review-execution-steps `reviewRound`/`reviewAdvance` 위의 executor-agnostic 라운드 루프(round→ready unit마다 executeUnit→advance→반복). **live**=executeUnit이 실 executor subprocess spawn, **mock**=fixture seat 기록. assemble은 호출자 몫(`ready_to_assemble` 반환). 반환 `ReferenceHostRunResult`{finalStatus(ready_to_assemble|halted|max_rounds), roundCount, executedUnitIds}. 결정론 mock E2E(review-api.test.ts): host 세션 prepare→driver(mock executor: lens markdown + finding-ledger 최소 valid yaml, maxRounds=2)→2라운드 결정론 전진(lens→finding-ledger, frontier가 finding-relation-graph까지). **`completed` 고정 record는 전체 파이프라인 필요(deliberation/synth/runtime-owned=4e 후속)이라 lens 슬라이스 밖**; 라운드 계약·A/B 분리·seat 계약은 agent 없이 증명됨. typecheck+full vitest 75files/889 green.
8. **rank-1 lexicon + rank-5 계약** 명문화. 게이트: 개념·계약 ↔ 런타임 대조. **[landed 2026-06-09]** rank-1 `core-lexicon.yaml`에 **`ReviewOrchestrationOwner`** 개념 등재(`ReviewReasoningUnitExecutionRoute` 직후): loop_driver/unit_dispatch/authority_invariant 속성 + runtime/host instance + **`distinction_from_route`**(route의 orchestration_locus='unit이 어디서'와 직교 명기) + execution_rules_ref(settings/stamp/guards/round-engine/host-tools/reference-host/contract 경로). rank-5 신규 `.onto/processes/review/host-orchestration-contract.md`: position(A/B 공유엔진)·orchestration 각인·라운드 계약(prepare→round→advance→assemble)·seat 기록 분업·구조 gate·A/B fail-closed 경계·controlled 심의·reference host·범위. **대조**: lexicon execution_rules_ref + 계약 §의 모든 파일/심볼 실재 확인(orchestration-owner guards·reviewRound/Advance·computeLensCompletionBarrier 등), onto allowlist PASS, YAML valid, citation-audit(lexicon 로더) 포함 75files/889 green.
9. **전체 검증**: typecheck + 전체 vitest(A 무회귀 0) + reference host live E2E(브랜드 1개) → `completed`. 보고. **[landed 2026-06-09]** 종합 그린: `check:ts-core`(typecheck) · full vitest **75 files / 889**(888 pass + 1 todo, 0 fail = A 무회귀) · reference host mock E2E · `build:ts-core`(패키지 빌드) · 정적 체크(retired-root-paths, onto allowlist[147], invocation-runner/mcp:review/review:route conformance) 전부 PASS. **완료기준 대조**: #3 A 무회귀 ✅, #4 locus 누수 0 ✅(orchestration-owner.test.ts 양방향), #5 host×nested 거부 ✅(settings superRefine), #6 typecheck/test + 개념·계약↔런타임 ✅. **#1·#2(live+mock → `completed` ReviewRecord)는 부분**: round/advance·seat·A/B·reference-host는 증명됐으나 `completed` 전체 파이프라인은 deliberation/synthesize/runtime-owned 단계의 host 구동(후속 추출)이 필요 — lens 슬라이스(사용자 승인 범위)는 lens→issue-artifact 결정론 전진까지. live E2E(`test:e2e`)는 실 LLM 필요로 미실행.
