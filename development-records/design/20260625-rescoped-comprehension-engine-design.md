...
- **필수 필드(최소)**: `region_identity`(시트·행범위·`columnResidualKey`) · `value_signature_tile_witness`(결정론) · `spine_claims`(structure/range/logic/semantics) · `examples`(canonical) · `semantic_depth`(+§3.4 라이프사이클 필드) · `capped_or_frontier_state` · `confidence_by_claim` · `is_lower_bound_by_claim` · `limiting_witness` · `provenance`(producer kind=deterministic/llm/vision-assist + epoch fingerprint 기여) · `consumer_handoff_notes`.
