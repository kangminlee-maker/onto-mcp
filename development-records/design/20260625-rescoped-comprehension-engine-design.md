...
**▶ Cut-4b 종합(4b-0/1/2)**: **공유 프레임-중립 spine(P8/Model A) = 검증** — synthetic(4b-1 one_spine_sufficient 4/4+5/5) + 실 데이터(4b-2b sufficient·faithful·honest·frame-neutrality 유지) 모두서 한 spine이 양 소비자에 충분; "frame-specific"은 *규범 판단 층*(소비자 몫·설계 정합)뿐. **"synthetic 소형" caveat = 실 101MB 워크북으로 실질 해소**. **reconstruct에 특히 가치**(현 실-파이프라인이 막힌 데이터→온톨로지 파싱을 spine이 제공·unblock). **잔여(전부 spine 밖·업스트림)**: 인벤토리 보강(cell 값·formula 텍스트·null rate·row count)·값-레벨 조인 확인(§5.6)·전체 14시트·게이트 미실행. **다음 = Cut-4b 교차검증 게이트(ultracode + onto) — owner go.**
