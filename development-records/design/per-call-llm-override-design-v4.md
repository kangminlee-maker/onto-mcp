...
- **[알려진 한계] cross-provider override + 유닛레벨 llm + continuation**: `applyProjectContinuationUnitPolicy`(review-api.ts:3847)가 continue/round/advance 시 `units`를 프로젝트 설정(비-overlaid)에서 재로드하므로, override로 시작한 리뷰를 continue하면 유닛이 구 provider로 되돌아가 mixed route가 될 수 있음. 공통 `onto_review` 일회성 경로는 무영향(actor+unit 모두 overlaid). 후속: continuation에 override(또는 effective unit selection) 재적용 or 해당 조합 fail-loud.
