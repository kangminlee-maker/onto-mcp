...
⏸️ **Step E — 잔여(quality 페이로드)**: leaf-read 잠정 라벨이 **authoring 프롬프트에 도달**(현재 산출·영속만·소비 0=품질 개선 미발생). `run.ts` 스프레드시트 프롬프트 투영부(~5966)에 leaf-read 라벨 섹션 주입(정직 태그·미독 명시·truncation) + 실 워크북 E2E(§6) + 측정 기록.
