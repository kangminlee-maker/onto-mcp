...
- **✅ Step D (R1/R2/R8/R3 실배선/CG-1) 완료 (2026-06-28·커밋)** — run.ts 라이브 배선. `ReconstructDirectiveAuthor.readLeafLabels?`(optional·direct-call 팩토리 구현=`callJsonAuthor` 경유→mock/real 자동) + **`runSpreadsheetLeafReadStage`**(export·`run.ts:11476 refreshAuthoredArtifactReuseMatch` 직전 호출): 저신뢰 스프레드시트 영역→`readLowConfidenceLeaf`→`buildLlmComprehensionArtifact`→**별도 sidecar 영속**(`<sessionRoot>/comprehension/<id>.leaf-read.yaml`·임베드 companion은 결정론 유지=R1). **R2** 집계 fingerprint를 `authoredArtifactReuseMatch.leaf_read_aggregate_fingerprint_sha256`에 fold(인스턴스 미fold). **R8** N-observation 순서무관 집계(sorted {observation_id, fingerprint} sha256). **R3 실배선**: `assertGatingKeyExcludesInEpochOutput`를 `sourceObservationsReuseSha256`·`authoredArtifactReuseMatch` 양쪽에 호출(ⓒ 누설 회귀 가드). **CG-1** `leaf_read` 카탈로그 등록(count 34→35). 검증=ts-core clean·**full vitest 2022 pass**(134 파일·회귀 0)·정적 게이트 6종·**stage 통합 테스트 4**(산출+sidecar 영속·model 회전·content 회전·no-op without readLeafLabels). **결정(owner: 비용 무관)**: 재호출 가드(resume 시 LLM 재콜 회피)=최적화라 이연(correctness=seed 키 fingerprint fold로 성립). 잔여 limitation=fingerprint는 초기 관측셋 1회 계산(재관측 라운드 미반영).
- **✅ Step E (quality 페이로드) 완료 (2026-06-28·커밋)** — leaf-read 잠정 라벨이 **모든 authoring 프롬프트에 도달**. `ObservationPromptPayloadOptions.provisionalLabelsByObservation`(렌더=`payload.provisional_labels{authority:'non_authoritative'·note:'hints, not facts'·bounded 64}`·`includeStructuralData` 게이트) + `ReconstructDirectiveAuthor.setLeafReadProvisionalLabels?`(팩토리 클로저 ref + 로컬 헬퍼 `projectObservationsForPrompt`로 13 호출부 일괄 주입) + runReconstruct가 stage 후 `spine_claims`→`col<i>: <label>` 맵을 author에 set. **★reuse 키 미접촉**(R3 안전·R1 정합): `sourceObservationsReuseSha256`는 고정 필드 subset만 직렬화→라벨은 프롬프트 텍스트에만·아티팩트/키엔 0. 검증=ts-core clean·**full vitest 2025 pass**(134 파일·회귀0)·정적 게이트 6종·Step E 테스트 3(라벨 도달·미제공시 omit·타 observation 미부착). **정직 갭**: mock은 라벨 무시→실 품질 개선=미측정(실 LLM·월 한도)·실 워크북 E2E 미실행. **▶ P1-C2-A 빌드 A-E 전부 완료. 다음=실 LLM 품질 측정(천장 차이 최종 입증·한도 회복 후).**
