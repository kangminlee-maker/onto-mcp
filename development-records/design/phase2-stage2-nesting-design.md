...
> Landed 요약: Step 1 `review/nesting-batch.ts`(1cac0ce) · Step 2 codex outer 정합(8de6cdf) · Step 3 claude outer(b7dc8d0) · Step 4 A 통합+fail-closed 해제(1fcdad4) · Step 5 B 통합+settings 해제+completed E2E(17373d6) · Step 6 4셀 동등성 게이트(2d2fe8f) · Step 7–8 계약·lexicon 명문화+전체 검증(1cffdc5). 상세는 각 커밋 메시지가 권위.
> **후속 landed — A downstream nesting (2026-06-10, 사용자 요청으로 §6 비범위에서 승격)**: DS1 wave 분할 `dispatch_width`(e476399) · DS2 transport core `dispatchNestedBatch` 추출(a33c4fb) · DS3 A wide 3단계(issue-stance·per-issue deliberation·per-issue synthesis) nested 1차 시도+flat retry fallback(5b2b954) · DS4 계약·lexicon 갱신(이 커밋). 단일-유닛 체인 단계는 의도적으로 flat 유지(fan-out 부재). 계약 §4 갱신본이 권위.
