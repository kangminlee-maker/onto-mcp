...
- **다중 순차 호출 + 전부 한 출력에 렌더링** — 세션 `019fa332-ae9e-…`:
  ...
  **리뷰어 F1의 시퀀스가 그대로 실측돼 있다** — 전송분 4 : 수신분 1이므로 서수 짝짓기는 여기서
  즉시 깨진다.
- **다중 호출 + 페이로드가 출력에 전혀 안 들어감** — 세션 `019fa334-7926-…`:
  `r0..r3` 4회 fetch 후 `store("probe2", r2); text("done")` → 수신분은 **4자(`done`)**.
  MCP 결과는 exec의 JS 런타임에는 있었지만 **모델 컨텍스트에는 안 들어갔다.** 이 경우
  `delivered=0`은 **참인 진술**이다(§12-S3이 걱정한 "거짓 진술"과는 다른 경우다 — S3은 모델이
  본문만 변형해 출력하는 경우이고, 여기는 아예 출력하지 않은 경우다).
- **호출 0회 exec** — 15건.
