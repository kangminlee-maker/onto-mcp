...
> **종결(2026-07-29) — 셋 다 라이브 확보 없이 닫혔다. 측정이 아니라 구조로.**
>
> 켜기 전 차단 요인으로 계상해 두었으나, 판정이 실제로 무엇에 의존하는지 코드로 다시 도출하니
> 세 위상 모두 **판정이 보지 않는 축**에서만 달라진다.
>
> - **다중 `text()`** — 우리 façade는 성공·실패 양쪽 모두 `text` 파트를 **정확히 하나** 반환한다
>   (`observation-read-facade.ts:623`, `:816`). 재조정은 비교 전에 우리 서버로 스코프하므로 다중
>   파트는 우리 레코드에 나타날 수 없다. 이 불변식은 어디에도 고정돼 있지 않았고 — 리더가 구분자
>   **없이** 이어붙이므로(`textOfContentArray`) 파트가 둘이 되면 정본이 안 맞아 **일어난 배달을
>   `not_attested`로** 보고했을 것이다 — 이제 테스트가 두 경로를 함께 고정한다.
> - **동시 호출 · 겹친 exec** — 둘 다 **순서와 묶임**만 바꾼다. 판정은 셋에만 의존하고 전부
>   순서·짝짓기 무관이다: 전송분 **다중집합** 대조 · 전 수신분을 훑는 포함 검사 · 순서 비의존 fold
>   (단계 0b). **`call_id`는 리더가 나르기만 하고 `delivery-reconciliation.ts`가 한 번도 읽지
>   않는다** — §9-F1이 짝짓기를 제거한 결과가 코드에 그대로 있다.
>   → 라이브로 그 모양을 사는 대신, **그 무관함 자체**를 고정했다: 페이지를 여러 수신 레코드에
>   흩고 `call_id`를 공유시키고 순서를 뒤집어도 판정과 attestation이 동일해야 한다
>   (`delivery-reconciliation.test.ts`). 변이(수신 레코드가 하나라고 가정)가 이 테스트를 깨뜨린다.
>
> **남는 것**: 이 결론은 "codex가 MCP 결과를 `custom_tool_call_output`으로 렌더한다"는 전제 위에
> 서 있다. 그 전제가 깨지는 경우는 위상이 아니라 **버전 의미 드리프트**이며, 별도의 의미 프로브가
> 다룬다(그 설계 리뷰에서 이 종결도 함께 재검토한다).
