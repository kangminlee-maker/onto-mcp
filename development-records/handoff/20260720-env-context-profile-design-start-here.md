...
> **정정 (2026-07-20, 설계 종합 후):** 아래 "1차 의미 부여 = 결정론 대체재" 프레이밍은 **정정됨**.
> 환경 프로파일은 seed **생성**을 대체하지 않는다 — 구조 트랙(구현 구조·기술)만 다루며, 도메인
> 의미·seed 생성은 LLM 내용-읽기(의미 트랙)로 존속. 프로파일 기능 = (a) 생성-중 프로파일 disclosure
> + 주의 타깃팅(현재 설계), (b) 완성 seed의 구조 검증·보강(후속 워크스트림, flags-first). 보류된
> FD9/FD10/G11은 폐기 후보가 아니라 의미 트랙 본체로 존속. 종합·확정:
> `design/20260720-env-context-profile-crossverify-synthesis.md`.
