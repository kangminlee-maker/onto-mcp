...
- **S1 설계 착수**(이 핸드오프와 짝): `development-records/design/20260617-spreadsheet-extraction-adapter-s1-design.md`.
  **§8 = 대비 축: LLM-주도 적응적 관측 방법론**(LLM이 실행환경에서 추출 코드를 직접 작성·실행; runtime-owned 결정론 추출 S1과 대비).
  실증 사례 = 세션 `local_a1ae0b6b`(onto @0.4.12, **reconstruct 미사용**, openpyxl로 xlsx→온톨로지 산출).
