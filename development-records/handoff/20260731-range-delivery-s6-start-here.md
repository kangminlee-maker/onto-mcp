...
- **라이브 실측은 했다** (2026-07-31, `benchmark/20260731-range-delivery-live-probe/`). 실 codex 워커가
  façade를 띄우고 실 관찰을 가져갔고, 배달 레코드 v2에 실 구간이 실렸으며 `verbatim_delivered`가 나왔다.
  **다만 덮지 않은 것 셋**: 큰 관찰의 분할·재조립, **부분 인용**(이 트랙의 목적), S6의 judge 구간 투영
