...
- **라이브 실측은 했다** (2026-07-31, `benchmark/20260731-range-delivery-live-probe/`). 1차는 단일 페이지,
  2차는 **3페이지 walk**로 분할·재조립까지 덮었다. 남은 미실증은 **부분 인용**(이 트랙의 목적)과
  S6의 judge 구간 투영 — 진입점 `20260731-live-citation-arm-design-start-here.md`
- **⚠ 2차가 F-3을 라이브로 실증했다.** 수신 레코드 810/45,138/47,451자 — 개별 페이지는 예산 아래인데
  여러 페이지가 한 exec 출력으로 병합돼 잘렸고 **방출 3건 전부 미도달**이 됐다. 시스템은 옳게 동작했지만
  (인정 0, fail-closed), **다중 페이지를 한 턴에 가져가면 아무것도 인용할 수 없다.** 승격 전에
  "exec당 1회 유도"와 그 위반 시 동작을 정해야 한다
