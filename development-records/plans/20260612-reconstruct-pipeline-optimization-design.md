...
> **1차 슬라이스 (competency_questions, 구현됨)**: CQ는 현재 author→validate→**재시도 0회** hard-halt(라이브 `missing_required_coverage` 1/5). ontology_seed의 검증 실패 1회 재작성 패턴을 전체 재작성 방식으로 일반화: author input에 `repairAttempt` 추가, 배치 작성 단일 콜 지점(`callCompetencyQuestionBatch`)에 repair 프롬프트(미커버 coverage 지시문)·`CompetencyQuestionsValidationRepair` artifactName·userPayload `repair_attempt` 주입, run 흐름에 invalid→copy→repair 재작성→재검증 루프. 미커버 항목 추출은 순수 헬퍼 `competencyQuestionsRepairDirectives`(missing_required_coverage 우선 + 폴백, unit test). repair payload는 seed 패턴대로 이전 질문 coverage(`previous_questions_coverage`)·검증 요약(`previous_validation_summary`)을 투영해 통과 coverage 보존(onto material 지적 반영). repair 지시문은 systemPrompt 보간이 아니라 userPayload 구조화 데이터로만 전달(프롬프트 인젝션 차단, Codex P2). `CompetencyQuestionsValidationRepair`는 telemetry unit 매핑 등록(Codex P1). 검증: L1a 동일 철학(헬퍼 unit test + 전체 회귀 그린; mock은 항상 valid라 복구 경로는 live 확인 이연). **2차 슬라이스 (검증 실패 attempt 계측, 구현됨)**: seed·CQ repair 모두 검증 게이트 miss를 `attempts[]`에 미기록 → 복구된 run이 검증 게이트 miss를 숨겨 S1/S2 측정 부정확(Codex 이연 P2, 설계 §4 S2). 신설: shared `PipelineExecutionAttemptKind`에 `validation_gate` + 텔레메트리 콜렉터 `recordValidationGateFailure`(failed `schema_validation_failure` attempt 추가, `llm_call_count`·size 미증가, `attempt_count` 증가), seed·CQ invalid 분기에서 재작성 직전 호출(unified). **공유 ledger 배치는 의도적**: LLM 입출력·실패/재시도 특성은 파이프라인 공통 영역이고 LLM 응답 특성은 불가변이므로 공유 ledger가 그 현실에 맞춰 진화한다(reconstruct-local 회피 대상 아님). 따라서 `kind`/`failure_class`는 **추가형·전방호환 열린 집합**으로 계약에 명시(소비자는 미지 kind 관용, 추가는 schemaVersion bump 불요 — major bump는 추가형에 과신호; `PipelineId = ... | (string & {})` 선례). rule 2(출력 스키마 변경)는 사용자 확인으로 충족, Codex 버전-신호 P2는 전방호환 정책으로 해소. 검증: 콜렉터 unit test + seed repair 통합 테스트(invalid→repair→valid, `attempts[]`에 `validation_gate` 포함 확인 — seed repair 루프가 fixture로 E2E 검증됨을 재확인, CQ는 동일 패턴), 전체 회귀 그린. ledger contract 문서 개정. **남은 슬라이스**: ① ontology_seed semantic(기존 1회로 불충분 — bound·repair 컨텍스트 강화), ② final_output 잔여 의미 실패, ③ split배치 directive 필터링(Codex P2).
