/**
 * 실험2 처치 렌더 + 유효성 지표 (PROTOCOL.md) — live 세션 sidecar code 행을 DD9/DD10
 * 렌더러로 렌더하고 유효성 전제(admit ≥30 AND 라인 커버리지 ≥80%)를 기계 산출한다.
 *
 *   npx tsx development-records/benchmark/20260720-semantic-map-midfile-live/render-treatment.mts \
...
if (!sha.startsWith(TARGET_SHA_PREFIX)) {
  throw new Error(`exp2 target sha drift: ${sha.slice(0, 16)} — 렌더 무효.`);
}
